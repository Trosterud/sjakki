...
trait Square {
  ...
  def addRobot(arrivee: Nappula): Boolean
 ...
 * have a single `Wall` object that can be placed wherever there is an obstact in any
...
object Wall extends Square {
  ...
  def addRobot(arrivee: RobotBody) = {
    arrivee.destroy()//saapuja tuhotaan
    false
  }
